...
/**
 * Получает все активные посты с информацией о темах и авторах.
...
function getPosts(): array
{
    ...
    $sql = "SELECT 
            ...
            FROM `blog_task`.`post` AS `p`
            INNER JOIN `blog_task`.`post_theme` AS pt ON p.`theme_id` = pt.`id`
            INNER JOIN `blog_task`.`author` AS a ON p.`author_id` = a.`id`
            WHERE p.`status` = :status";

    $stmt = $pdo->prepare($sql);
    $stmt->bindValue(':status', STATUS_ACTIVE, PDO::PARAM_INT); // Используем bindValue для безопасной подстановки
    $stmt->execute();
